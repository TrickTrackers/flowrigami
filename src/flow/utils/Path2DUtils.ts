...
export function createRectanglePath2D(rectangle: Rectangle) {
  ...
  const curve = { x: x - halfWidth, y: y - halfHeight };
...
export function createMachinePath2D(rhombus: Rhombus) {
  const x = rhombus.x;
  const y = rhombus.y;
  const halfWidth = rhombus.width/2;
  const halfHeight = rhombus.height/2;
  ...
  const curve = { x: x - halfWidth, y: y - halfHeight };
  let path = new Path2D();
  let points:CoordinatePoint[]=[leftTop, rightTop, rightBottom, leftBottom];
  ...
  for (let i = 0; i < length; i++) {
    ...
    path.arcTo(point.x, point.y, (point.x + nextPoint.x)/2, (point.y + nextPoint.y)/2, 0);
  }
  path.lineTo(points[length - 1].x, points[length - 1].y);
  path.closePath();
  return path;
}
export function createMessagePath2D(rectangle: Rectangle){
  const x = rectangle.x;
  const y = rectangle.y;
  const halfWidth = rectangle.width/2;
  const halfHeight = rectangle.height/2;
  ...
  return createPath([leftTop, rightTop, rightBottom, leftBottom], rectangle.borderRadius);
}
